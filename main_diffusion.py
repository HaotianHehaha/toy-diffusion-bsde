...
folder_name_to = 'results/only_diffusion/beta=0.5'
...
input_size = 2
hidden_size = [10,32,32,64,64,32,32,10]
out_size = 2
time_steps = 100
learning_rate = 1e-3
num_epochs = 10000
...
gaussian_1 = torch.distributions.MultivariateNormal(mean1.to(device), cov1.to(device))
samples1 = gaussian_1.sample((3, ))
gaussian_2 = torch.distributions.MultivariateNormal(mean2.to(device), cov2.to(device))
samples2 = gaussian_2.sample((2, ))
gaussian_3 = torch.distributions.MultivariateNormal(mean3.to(device), cov3.to(device))
...
sample_true_train = sample_true.repeat((50,1)).to(device)
